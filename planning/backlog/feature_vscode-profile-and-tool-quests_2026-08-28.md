...
3. **It is not needed yet.** The Quest screen (CodeMirror + Pyodide) is already the
   gentle editor for Tiers 0–1, and Tier 0 wants no editor at all — the parent guide
   says Notepad is sufficient and that VS Code is Tier 2b vocabulary which "costs a
...
**A VS Code Profile**, exportable as one file and installed on his laptop at Tier 2b.
...
| Restored | At | Because |
|---|---|---|
| Breakpoints and the Run and Debug view | Tier 3 | nested loops and dict iteration are where stepping becomes revelatory |
| Outline, breadcrumbs | Tier 4 | files gain functions worth navigating between |
| Extensions view | Tier 6 | `dependencies` enters the curriculum |
| Problems panel | Tier 7 | ruff and pyright, and the Idiomatic medal |
| Source control view | Tier 7 | branches and pull requests — deliberately after the commands are muscle memory, so the GUI is a convenience and never a crutch |
...
`debugger` is registered as a **Tier 7** concept, so a Tier 3 debugging quest is
rejected by the validator's concept-above-tier rule. That rule is working as designed;
the fix is a decision, not a bypass. Preferred: add `breakpoints` at Tier 3 for
stepping and the Variables panel, and keep `debugger` at Tier 7 for the deep pass —
...
Tier 0's *errors are readable* promise made interactive, four tiers later, on the same
bug the Quest screen prototype already uses.

## Trigger for Promotion

**Tier 2b, week 7**, when VS Code enters the curriculum and the profile has to exist.
Authoring the profile is an hour; the ladder is authored per-tier alongside the quests
