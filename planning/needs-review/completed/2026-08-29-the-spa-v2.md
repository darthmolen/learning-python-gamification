...
**Status:** Planned
**Version:** v2 — revised 2026-08-29 after the first review
**Track:** spa
**Date:** 2026-08-28
...
**Reviewed:** 2026-08-29 12:38
**Reviewer:** Claude Code (plan-review-intake)

### Previous Issues — Resolution Status

1. **Resolved** — plan now explicitly requires binding the collection schema (`DueInvasionsSchema`) and explains why the entry schema cannot carry the §5.4 cap or dedup refinement.
2. **Resolved** — Phase 3 now cites `docs/design/pyquest/pyquest-campaign-ui.html` as the reference; Phase 4 has concrete deliverables, a named Pyodide boundary decision, and acceptance criteria.
3. **Resolved** — `src/gateway/` is built in Phase 2; screens may not import from `src/fixtures/` directly; a test asserts the rule.
4. **Resolved** — all nine screens and routes enumerated in route tables.
5. **Resolved** — `vitest.config.ts` already exists with aliases; plan corrects the v1 claim and identifies the real problem (`projects` entry for jsdom environment).
6. **Resolved** — Phase 1 now says `[no upstream dependency]` and lists scaffold steps explicitly.
7. **Resolved** — dependency graph stated phase by phase.
8. **Partially resolved** — offline deferred to backlog with clear reasoning (stack is always available by household arrangement). Quest/Submit degraded-state UX remains unspecified, but the scope decision is documented.
9. **Resolved** — the son's laptop check is a named Phase 2 exit condition.
10. **Resolved** — keyboard/accessibility is a stated success criterion.

### Self-Audit Flag

The plan correctly flags that touching `pyquest/vitest.config.ts` breaks its own track-discipline claim. Grounding confirms: the file exists, has no `projects` entry yet, and no other in-progress plan lists it. The plan's additive-and-no-other-track-has-one claim is accurate *today*, but "no other track has a projects entry" is a runtime observation, not a structural guarantee. The `api` and `db` tracks run in the same workspace and may need jsdom entries too. **This is manageable, not blocking** — but the disjointness claim should be weakened from "can run at once" to "can run at once with explicit coordination on the one shared file."

### New Issues

#### Important (Should Address)

- **Track discipline / "all three Lane A tracks can run at once"**
  - `pyquest/vitest.config.ts` is a shared root file; the claim of full disjointness is now only true if `api` and `db` tracks do not also need it. It should be stated as a coordination point, not implied as resolved.
  - Suggested fix: Amend track discipline to say this track makes the first `projects` entry and any later track that needs one must coordinate rather than assuming disjointness.

- **Phase 2 exit condition / the son's laptop failure path**
  - Phase 2's the son's laptop check is the exit condition, but the plan does not say what happens if the check fails — does Phase 2 block, does design defer, or is it a known-acceptable shortfall?
  - Suggested fix: Add one line: if the son's laptop check reveals a layout problem, open a backlog item and note it in the Phase 2 review; this plan does not block on it.

### Assessment

**Implementable as written?** Yes

**Reasoning:** All critical issues from v1 are resolved and the plan is grounded and actionable; the two remaining items are coordination notes rather than blockers.
...
"Twice reviewed, and offline leaves v1" names one outcome; the rest of both rounds is not recorded.
