...
**Reviewed:** 2026-09-01 18:41
**Reviewer:** Claude Code (plan-review-intake)

### Strengths

- **Why this is one plan and not three / One Dockerfile shape, three services:** Good root-cause framing. The plan correctly ties `api`, `web`, and `migrate` to the same Windows bind-mount/node_modules failure already documented in `infra/compose/api.yml`.
- **Success Criteria / Phases:** Clear sequencing; starting with `migrate` is a sensible smallest proof.
- **Architecture:** Reuses the existing "build inside image, mount source" pattern from `pyquest/apps/runner/Dockerfile`; keeps content/progress separation intact; no obvious secret-in-Dockerfile risk.
...
- **Dependencies / Prerequisites**
  - **Problem:** The plan says "None blocking," but `apps/web/**` is owned by the `spa` track while this plan adds `pyquest/apps/web/Dockerfile`.
  - **Why it matters:** Conflicts with the repo's one-plan-per-track/file-ownership convention in `CLAUDE.md`.
  - **Suggested fix:** Get explicit SPA-track signoff or split/retarget ownership; the plan already notes this deserves "a glance from that track's owner" — that glance needs to be a formal signoff.

- **Approach / One Dockerfile shape, three services**
  - **Problem:** "Mount source only" is underspecified for Node. A plain bind mount of `../../pyquest:/workspace` will shadow image-built `/workspace/node_modules`, defeating the fix.
  - **Why it matters:** Without an explicit compose layout that prevents the mount from hiding the image's `node_modules`, the proposed fix can still fail in exactly the same way.
  - **Suggested fix:** Specify the exact mount strategy (e.g., a named volume or exclusion path) that preserves container-installed dependencies while mounting source.

#### Important (Should Address)

- **Success Criteria**
  - **Problem:** "`npm test`, `npm run typecheck` and `validate:content` unaffected and clean" is not concrete enough for verification.
  - **Suggested fix:** Name the exact commands and working directory (`pyquest/`) per `CLAUDE.md` conventions.

- **Approach / `bounce.cmd <profile>`**
  - **Problem:** `docker compose --profile <p> up ... <services>` cannot infer `<services>` from profile alone; the script contract is incomplete.
  - **Suggested fix:** Define the profile→service mapping explicitly in the plan.

- **Approach / `start-full.cmd`**
  - **Problem:** Lacks first-time setup/error behavior (`.env` missing, migration failure, unhealthy service).
  - **Suggested fix:** Specify non-zero exits and preflight checks.

- **Consistency / `smoke.sh` health assertion**
  - **Problem:** The plan does not name which service endpoint it will assert on. `api` likely has `/health`; `web` is less obvious.
  - **Suggested fix:** Name the target service and health endpoint the new assertion will check.

#### Minor (Consider)

- **Files Expected to Change**
  - `infra/README.md` is listed, but the plan omits whether new build args or env vars require changes to `.env.example` or compose root docs.
  - **Suggested fix:** Verify whether new build args/envs are needed and list those files if so.

### Recommendations

Add exact compose changes, exact healthcheck endpoints, explicit script failure modes, and explicit profile→service mapping. Reconcile `apps/web/Dockerfile` with `spa` track ownership before implementation.

### Assessment

**Implementable as written?** With fixes

**Reasoning:** The diagnosis is strong and the phase order is good, but the core bind-mount/node_modules mechanics and `bounce.cmd` service targeting are too implicit, which risks rework on the primary fix.
