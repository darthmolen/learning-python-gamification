...
description: Records what a plan review actually decided — accepted, merged, rejected, flagged — so the reasoning outlives the conversation. Use immediately after evaluating a review, before moving on. Closes the gap where a review is acted on and nothing in the repository says which findings were taken or why the others were not.
metadata:
...
A review pipeline copies a plan out, gets it critiqued, and hands the findings back. The author
evaluates each one and reaches a disposition — **accept**, **merge**, **reject**, **flag**. Then
the conversation ends, and unless somebody writes the outcome down, it is gone.

**That is a real hole.** The review file survives, the revised plan survives, and the mapping
between them — which finding produced which change, and which was argued down — does not. In the
repository this skill was written for, six of seventeen filed reviews recorded nothing at all
about their outcome.
...
Appended to the plan itself, one paragraph per round, in its source location — **not** in the
review copy, which is an audit trail and is never edited.
...
Two rejections from this skill's home repository were only defensible because somebody ran the
command: a "wrong test count" that turned out to be a static count of `it` blocks, missing a
parameterised case that expands at run time — and a request to add a lint step to a checklist,
when no linter was configured anywhere in the workspace. Both readings were reasonable from the
text alone. Neither survived thirty seconds in a terminal.

## When a review is rejected wholesale

Still record it. `[PLAN] <plan> v2 review returned; both findings rejected, no changes` is a good
commit — it says a review happened, was taken seriously, and changed nothing, which is a
