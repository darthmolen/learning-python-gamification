...
Question 1 was also asked in session 4. That is deliberate — it is the concept tonight
...
ago: `spare = inventory[:]` is a copy where `spare = inventory` is a second name. Session 3's
...
### `s6e1_the_hotbar.py` — the slice, and what it leaves behind
...
Task 4 is the session-3 callback: `inventory[:]` against `inventory`, appending to each,
printing the original both times. Only one of them is a copy.

### `s6e2_past_the_end.py` — why one of these crashes and the other does not

In session 2, reaching past the end was an `IndexError`. Tonight it is not, and the
