...
The second half of the area starts here, and the vehicle changes shape. Sessions 1–7 were
...
If the evening is full, `s8e3` and the walkthrough can move to session 9 without damage.
...
### `s8e1_things_by_name.py` — a key, and the value it points at

Making one, reading one, changing one, adding to one. Task 3 asks what shape would hold two
recipes at once — they are describing session 12 and should be told so, because knowing the
road continues is worth more than the answer.

### `s8e2_the_key_that_is_not_there.py` — the bug the rest of the session hunts
...
### `s8e3_stop_and_look.py` — `breakpoint()`, the built-in
