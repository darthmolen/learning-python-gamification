...
the two heaviest. Sessions 3 and 4 were about invisible behavior; tonight everything does
...
Some of them will already have `in`, because session 4's task 3 handed it to them. Good —
...
is what makes the short way mean anything, and `s5e2` has both side by side for exactly that
reason.
...
### `s5e1_do_i_have_enough.py` — `in`, and the crash that stops happening

Walks a list of needed things against a list of carried things. Task 4 sends them back to
`s4e2_the_missing_pickaxe.py` to guard its deliberate crash with `in` — **fixing a file that
...
### `s5e2_the_tallest_tower.py` — one idea, asked in both directions
...
### `s5e3_tidy_it_up.py` — the two sorts, settled

`sorted()` versus `.sort()`, side by side, after session 3 introduced the trap. The question
...
**Do not skip the long-way loop in `s5e2` for time.** Deleting it turns `max()` from a
