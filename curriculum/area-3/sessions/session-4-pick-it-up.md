...
something to do, which is the right order: session 3 named the behavior, session 4 uses it.
...
Question 3 is planted for session 6 and has nothing to do with tonight. That is rule 3 of
...
### `s4e1_pick_it_up.py` — the five orders, one after another
...
This is the area's **second** silent failure, after session 3's aliasing, and it is worth
ten minutes even if it costs the choice board.

### `s4e2_the_missing_pickaxe.py` — `ValueError`, on purpose
...
Task 3 guards it with `if "pickaxe" in inventory:`. **`in` is session 5's material arriving
