...
question. Tonight teaches the shape; session 11 is where it earns its place.

The DC band steps up here — sessions 10 to 13 sit at 18–22 — not because sets are hard but
...
Question 3 is needed tonight: a set can only hold things that cannot change, and `s10e2`
puts tuples in one.
...
### `s10e1_a_bag_with_no_order.py` — the property, then the job
...
Task 3 tries to put a list inside a set and fails. That error is the setup for `s10e2`.
...
### `s10e2_seen_it_before.py` — a set as memory, and blocks that build once
