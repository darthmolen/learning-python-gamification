...
and cutting it short costs nothing. If the evening has room, the fourth task in `s7e1` is
one of the better arguments in the area.

The half-area ends here. Sessions 8 onward are dicts, and the vehicle stops being a row of
...
Point out that they have been handling coordinates since session 1: `place(x, y, z, kind)`
...
### `s7e1_a_coordinate.py` — tuples, and a list of them
...
### `s7e2_it_will_not_change.py` — the refusal, on purpose
...
`total = total + 1` does to a number — session 3's idea, arriving for the third time and
