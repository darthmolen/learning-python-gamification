...
### `p1e1_a_row_of_blocks.py` — a list becomes a row
...
### `p1e2_the_floor.py` — a loop inside a loop, and four hundred blocks

Nested loops are Area 1 practice 7 vocabulary, so this is a resurfacing rather than a new
...
- **A checkerboard.** Two kinds alternating across the floor. They will need the `%` from
  `p1e2` and should be made to say what it is doing.
...
4. **They copy-paste a second row rather than nesting.** Let them, once. `p1e2` is the
