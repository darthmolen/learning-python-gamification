...
resurfacing
**Files:** `practices/practice-11/`
**Journal:** entry 35

**This session introduces nothing, and that is its whole design.**

Set subtraction does in one character what a six-line loop did in practice 5. The two are put
...
2. In practice 5, how did you find what was missing from a list?
...
Do not write any new syntax yet. Ask them to write practice 5's loop again, from memory, on
paper:
...
### `p11e1_what_am_i_missing.py` — both versions, proven equal
...
### `p11e2_pick_the_right_one.py` — three questions, three shapes
...
to `need - have` and it works, ask for practice 5's loop anyway. The pairing is the entire
