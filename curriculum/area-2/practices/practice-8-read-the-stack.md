...
**Files:** `practices/practice-8/`, and `reference/practice-8-answers.md` — **yours, read
before the session**
...
**Read `reference/practice-8-answers.md` before he sits down**, and do not have it open in
...
- **Area 0 Practice 3** was `reading-errors`: one or two frames, in tiny files.
- **Area 1 Practice 6** was the same skill against loop errors.
...
### `practices/practice-8/w8_read_the_stack.md`
...
- **Step 7 — the main-guard, and it must be done in the given order.** Run `banner.py`,
  ...
  `__name__` is the second word Python fills in for him after `__file__` in Practice 5, and
  it holds one of exactly two things. That is the whole explanation and there is no need
  for more of one tonight.

- **Step 8 — the six written questions.** Answers in `reference/practice-8-answers.md`.
...
- **The Fourth Floor** — add a `deeper_frame.py` to the chain and predict the traceback
  before running it.
- **The Own Goal** — break one of his Practice 5 or 7 files on purpose, in a way that
