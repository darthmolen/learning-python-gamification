...
that is why `git log` is interesting to him at all in Practice 3.
...
### `practices/practice-1/w1_the_folder_that_remembers.md`
...
what is in `HEAD`, and what goes in `objects`. Practice 3 checks both.
...
- **The Spelunk** — go through every name in `.git` and write one guess per name about
  what it is for. Practice 3 marks them.
