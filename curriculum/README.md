...
`packages/content/` and is authored separately. Each tier's README records which of its
exercises should later become quests, and of what kind.

| Tier | Weeks | Subject | Status |
|---|---|---|---|
| [tier-0](tier-0/) | 1–2 | First Light — `print`, variables, the four types, `input`, f-strings, reading errors | **authored** |
| tier-1 | 3–6 | Control — `if`/`while`/`for`, nesting, the accumulator pattern | not started |
...
- **Every tier has a `verify.py`** that runs every exercise and checks it against its own
