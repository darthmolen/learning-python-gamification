...
**Everything goes in one `journal.md`**, at the top level of his own repository. One entry per
session, newest at the bottom, each one starting with a dated heading copied from
`../TEMPLATE.md`:
...
The date on the heading is what the game reads to tell one entry from the next, and it is the
same date the ledger pays against — so `2026-10-19`, never `Oct 19`. The session number after it
is for him; the date is for both of you.

This directory goes the next time somebody is editing Area 1 for another reason.
