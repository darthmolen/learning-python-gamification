...
Tonight he deletes typing he has already done, and gets a machine out of it.
...
Open `s4e2_the_dashed_orbit.py` from Area 0. His file. Scroll through the four
identical blocks of four lines. Ask him what number he wrote in his Journal when
that file asked him to count the blocks he had pasted.

Then show him, on screen, three lines:
...
Task 1 asks what the first number printed was. **Do not let him skip it.** The answer is
0 and it is the setup for the next file and for the next four weeks.

Tasks 2 and 3 are the pentagon. He changes 4 to 3 and gets an open shape; then he has to
work out the turn himself. Let him guess. 90, then 100, then something. The question when
he stalls:
...
Four loops, four written predictions before he runs anything. Enforce it; it costs ninety
seconds and it is the whole mechanism.
...
he will meet properly in session 6.

Task 3 is the gap in the spoke drawing: the first spoke has length 0 and draws nothing.
There are two fixes and one of them changes a single number. Ask which one he would
...
`turn = 360 / sides`, and everything downstream follows. He changes one number at the top
and gets a triangle, a pentagon, an octagon, and at 36 sides a circle he never asked for.

**Say this out loud when the circle appears:**

> "You did not type the word circle anywhere in that file."

**Tell him to keep this file.** It comes back in session 7 and session 9, and it is the
...
colour for shapes with more than six sides — is deliberately impossible tonight. If he
picks it, let him try, then have him write in the Journal what he wanted to say and could
not. Session 4 gives him the word.

---

## Beat 5 — Journal (5 minutes)

Entry 07, and there is one thing to say before he writes it. Thirty seconds, once:
...
The thing worth capturing tonight: **how many lines the loop deleted.** He has the number
from Area 0.
...
**He reads `range(4)` as "1 to 4".** He is right about the count and wrong about the
numbers, and it will cost him in session 8 if it survives. *"Print every number it gives
you. What is the first one?"*

**He puts `turtle.done()` inside the loop.** Indentation means something now. *"Which lines
are inside the loop? Read me only those."*

## Success condition

By the end of tonight he can write a `for` loop from scratch, say what `range(6)` gives,
...
and watch whether he reaches for the calculator or the rule.
