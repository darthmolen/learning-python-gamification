...
Read entry 07's *what will break next time* back to him. Say what actually happened. Two
...
Say why it is lopsided, because he will ask and "that is just how it is" is a bad answer:
...
Do not demonstrate the third form. He meets it in the file.
...
Five loops, five written predictions. He scores himself out of five and the score goes in
the Journal, because he does this again in session 8 and comparing is the point.

The one to sit on is `range(10, 0)`, which prints **nothing at all** and does not
complain. That is the second time he has met a loop that silently does not run. Do not
...
he should find it rather than be told.
...
`input` from Area 0 meets the polygon engine from session 1. One trap and he has met it
before: `input` hands back a `str`, and `forward("90")` is a `TypeError` from Area 0
session 3.

Task 3 asks him to answer 1 and then 0. One crashes and one does something strange but
legal. **He cannot fix either tonight and both go in the Journal.** Session 5 fixes both,
and finding his own note from three sessions ago is the point of writing it down.

---

## Beat 5 — Journal (5 minutes)

Entry 08. The thing worth capturing: **his score out of five on the predictions**, and
which one he got wrong.
...
**`range(2, 10)` gives 8 numbers and he expected 9.** The most common off-by-one in the
language. *"Write down what you think it gives. Then print it. Which end did you get
wrong?"*

**He changes `sides` and the turn does not follow**, because he typed the turn as a
...
**Let him get wrong:** the hard-coded turn. The fix — `turn = 360 / sides` — is a line he
should write himself, out of irritation.

**Say nothing** when he copies a loop three times to draw three shapes. Say nothing at
all. Session 7 lands much harder if he has felt this first.

## Compression note

If the calendar bites, this session merges into session 1. Keep `s2e1`'s five predictions
and `s2e3`'s dial; `s2e2` can go to his own time.

## Success condition

He can write `range(start, stop, step)` without looking it up, say which end is included,
