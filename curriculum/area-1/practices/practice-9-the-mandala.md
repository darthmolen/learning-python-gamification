...
**Files:** `practices/practice-9/`
**Journal:** entry 15

**This is the whole session.** No teaching, no worked example, no new orders. Somebody
builds something.

It is the rehearsal for Boss 1, exactly as Area 0's Practice 6 was for Boss 0, and it is
...
`p9_mandala_lab.py` runs, draws one plain ring, prints one number, and has two dials at
...
`turtle.speed(0)` has been available since Practice 1. If the drawing passes about ten
...
Say so explicitly if they hesitate. `p1e3`, `p7e3` and `p8e2` are the three worth opening,
...
**Blank page.** Scope, exactly as in Area 0 Practice 6. *"One ring. Just one. Don't design
the rest yet."*
...
All three matter, and the third one is the one that makes Practice 10 work.
