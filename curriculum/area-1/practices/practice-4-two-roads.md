...
`if` arrives now rather than in Practice 1 on purpose. A conditional with nothing to
...
Practice 3, all six of them — and what it hands back is a `bool`, which is the fourth kind
...
is the tricky part**, and if it defeats them tonight, Practice 5 gives them the cleaner way
...
If the calendar bites, this session merges into Practice 5, which teaches the same material
under more pressure. Keep `p4e1`'s boundary task and `p4e2`'s indentation task; `p4e3`
folds into Practice 5's gatekeeper.
