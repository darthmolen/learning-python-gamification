...
difference between this session and Area 0's Practice 3. There, every failure announced
...
edit in `practice-6/` and check these still match — this exact drift happened twice while
...
**Silent, and it hangs.** They know the escape hatch from Practice 3: Ctrl-C in the
terminal.
...
- **No window opened.** Not for a moment. This is the `SyntaxError`-family behavior from
  Area 0 Practice 3: Python reads the whole file before running any of it, so it never got
  as far as line one.
...
number, and Area 0 Practice 4 spent a whole evening on exactly that distinction.
...
Area 0 Practice 3 made the same point about `turtel`.
...
The fix is `turn = 360 / sides`, which is the rule from Practice 1 that they have used in
...
Same as Area 0 Practice 3, with one rule added: **at least two of their three bugs must be
