"""Run every Tier 0 exercise and check it does what its header says it does.
...
# Mirrors the tier-0 entries of packages/content/src/concepts.ts. Kept here as a
...
def check(path):
    ...
    unknown = [c for c in named if c not in TIER_0_CONCEPTS]
    if unknown:
        # Either a typo, or vocabulary from a tier he has not reached. Both are
        # authoring bugs and both are silent without this check.
        return False, f"concepts not in the Tier 0 registry: {unknown}"
