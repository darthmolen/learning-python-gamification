...
Broken sigil 6 from session 3, promoted to a fix-it quest. `int("ten")` is
...
line"*. So these check both — that the ValueError is gone, and that the behaviour the broken line
...
def test_int_is_no_longer_asked_to_read_a_word_as_a_number() -> None:
    """A ValueError is the right kind of thing carrying content it cannot use."""
    _, fell_out = attempt()
    assert not isinstance(fell_out, ValueError), (
        f"still a ValueError — {describe(fell_out)}. int() was willing to look at what it "
        "was given and could not read it. What would it have accepted?"
    )
...
def test_the_turtle_still_moves_the_line_was_fixed_not_deleted() -> None:
    """Deleting `size` trades a ValueError for a NameError; deleting both draws nothing."""
    spy, _ = attempt()
    assert spy.times("forward", "fd") == 1, (
        "the turtle never moved. Both lines stay — the size has to be worked out and the "
        "turtle has to be told to walk it."
    )


def test_forward_was_given_a_number_not_a_piece_of_text() -> None:
    """`size = "10"` also silences the ValueError, and it is the wrong lesson twice over."""
    spy, _ = attempt()
    given = spy.given("forward", "fd")
    assert given, "forward() was never called"
    assert is_a_number(given[0]), (
        f"forward() got {given[0]!r}, a {type(given[0]).__name__} — the size has to be a "
        "number by the time the turtle is handed it"
    )


def test_the_size_is_still_ten() -> None:
    """The brief says ten. A number that runs is not the same as the number that was asked for."""
    spy, _ = attempt()
    assert spy.given("forward", "fd") == [10], (
        f"expected ten steps, got {spy.given('forward', 'fd')}"
    )
