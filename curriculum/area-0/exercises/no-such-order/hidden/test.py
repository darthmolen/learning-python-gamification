...
Broken sigil 5 from session 3, promoted to a fix-it quest. The starter says
...
The stub reproduces that refusal rather than answering to any name at all — see
`ORDERS` below. A yes-man stub would pass the broken starter untouched.
...
def test_turtle_is_no_longer_given_an_order_it_does_not_have() -> None:
    """Not a NameError: Python found `turtle`. What came after the dot is what failed."""
    _, fell_out = attempt()
    assert not isinstance(fell_out, AttributeError), (
        f"still an AttributeError — {describe(fell_out)}. Python found turtle and then "
        "could not find that order on it. Read the spelling after the dot."
    )
...
def test_the_second_side_is_still_drawn_the_order_was_fixed_not_deleted() -> None:
    """Deleting the misspelled order silences the error and halves the drawing."""
    spy, _ = attempt()
    assert spy.times("forward", "fd") == 2, (
        f"expected two forward orders, found {spy.times('forward', 'fd')} — deleting the "
        "broken one removes the error and the second side with it"
    )
