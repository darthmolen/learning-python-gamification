...
Practice 3's fix-it exercise, promoted to a quest. One line starts in
...
line"*. So these check both — that the IndentationError is gone, and that the behavior the broken line
...
def test_the_stray_indent_is_gone_and_the_file_compiles_now() -> None:
    """Blank space at the start of a line is not decoration, and Python names it precisely."""
    _, fell_out = attempt()
    assert not isinstance(fell_out, IndentationError), (
        f"still an IndentationError — {describe(fell_out)}. Every one of these lines has "
...
def test_the_corner_is_still_turned_the_line_was_moved_not_deleted() -> None:
    """Deleting the indented line also makes the file compile. It is not moving the line."""
    spy, _ = attempt()
    assert spy.times("left", "lt") == 1, (
        "the turtle never turned. Deleting the indented line removes the error and the "
        "corner together — move it back into line instead."
    )


def test_the_turn_is_still_ninety_degrees() -> None:
    spy, _ = attempt()
