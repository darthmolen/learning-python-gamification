...
def test_the_misspelled_name_no_longer_raises_a_name_error() -> None:
    """`turtel` is not `turtle`. Python looked for that name and there was nothing under it."""
    _, fell_out = attempt()
    assert not isinstance(fell_out, NameError), (
        f"still a NameError — {describe(fell_out)}. Read the name Python could not find, "
        "then look at the line it names."
    )
...
def test_the_corner_is_still_turned_the_line_was_fixed_not_deleted() -> None:
    ...
    assert spy.times("left", "lt") == 1, (
        "the turtle never turned. Deleting the broken line makes the error go away and "
        "leaves one long straight line — fix the line instead of removing it."
    )
