...
It is also the one that makes session 5 possible, because `input` hands back a `str` and
nothing in session 5 works until they know what that means.
...
Then, if they shipped variables called `a` and `b` in session 2: open that file and ask
...
3. **`int("12.5")`** — predict, then run. It raises `ValueError`, met as b6.
   ...
   This is the exact bug that will bite them in session 5.
...
Do not fight to make tonight great at the cost of them arriving at session 5 sour.
