...
that is his. Area 2a turns that folder into a git repository; until then it is just a
...
**Rung 3 — instrument.** Hands him a tool rather than a fact. In Area 0 the answer to
...
| Crashes when he types "big" | Correct behaviour, and he cannot fix it yet | "Is that your bug or the user's? What would you *want* to happen? Hold that thought until Area 1." |

**Let him get wrong:** the crash on bad input. He cannot fix it — the fix needs `if` and
`try`, which are Areas 1 and 5. Let it stand as an open wound. Write it into the
...
his folder. From Area 2a it becomes a comment in Gitea and turns into code-review
...
By the end of Area 0 he should be able to read an error message out loud, say what kind
