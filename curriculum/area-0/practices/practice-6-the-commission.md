...
**Files:** `practices/practice-6/` — `commission-brief.md` and `p6_starter.py`
...
`p6_starter.py` is deliberately almost empty. It runs, and it lists every order they own
...
If they want to copy chunks out of Practice 5's files, let them. That is what everybody
...
the staircase in Practice 2, and the concepts they are shaky on come back as invasions
