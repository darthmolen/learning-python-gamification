...
**Files:** `practices/practice-2/`
**Journal:** entry 2

Variables are usually taught as "a box that holds a value". That metaphor breaks by
Area 3 and has to be unlearned. This session teaches them as **names**, and teaches them
by first making them feel the pain of not having any.
...
If they cannot answer one, do not tell them — ask where they could find out, then move
...
**Give them the problem before you give them the tool.** Ask them to type this, by hand,
no copy-paste:
...
They edit four numbers. Let them. Then: **"Now 220."** They edit four more. Then, before
they finish: **"Actually, 175."**

Somewhere in there they will miss one, and the square will not close, and it will look
stupid. That is the lesson and it cost you ninety seconds of their irritation.

Then ask the question:

> "How many places is that number written? How many places *should* it be written?"

Let them answer. Then show one line:

```python
side = 150
```

and let them work out the rest themselves. They will. They have seen variables in
Scratch, and this is the same idea with better syntax.
...
They will read it as maths and object that it is false. **They are right that it is false
as maths.** Say so. Then ask what happens first — the left side or the right side.

The file prints `step` after every stair, so the answer is on screen the moment they run
it. Do not narrate the answer; make them read the output.

**Task 2 in that file is a trap you are setting on purpose.** It asks them to add two
more stairs, and it costs them six lines of typing to gain two stairs. Make sure they
notice. Then say:
...
That is Area 1 being sold in advance, and anticipation is a design goal — spec §5.2 keeps
...
explain floats tonight.** Let the decimals stand, let them be mildly puzzled, and say
Practice 4 is about that. A question they are already carrying is worth more than an
...
That last option is on every choice board in this area, and it should be. Autonomy is
...
Entry 2. Same four prompts. Open by reading them what they predicted last time.

If they wrote "nothing broke", ask what happened when the square did not close during the
...
2. Renaming a variable in some places but not all. The error says so. Make them read it.
3. `print("side")` versus `print(side)`. Worth doing deliberately if they do not do it
   by accident.
4. Naming things `a`, `b`, `c`. **Let this one ship.** Open the file at the start of
   Practice 4 and ask them what `b` was.

## What you may not say

When the square does not close because they missed one of the four numbers: do not point
