...
It is also the one that makes Practice 5 possible, because `input` hands back a `str` and
nothing in Practice 5 works until they know what that means.

**Design note, and the reason for the shape below:** types are taught here as *an
answer to a question they already have*. They have already met three of the four by
accident —
the string in `color("red")`, the decimals from the 1.5 staircase in Practice 2, and the
...
Then, if they shipped variables called `a` and `b` in Practice 2: open that file and ask
them what `b` was. Say nothing else. The silence does the teaching.

---

## Beat 2 — The hook (10 minutes)

Start with their own bug, from last session. At the `>>>` prompt:

```python
turtle.forward("100")
```

They know this one. Ask them why it fails, and they will say something like *"because
it's in quotes"*. Push once: **"What difference do the quotes make? It's still 100."**

Then hand them the instrument:
...
`<class 'int'>` and `<class 'str'>`. Warn them in advance that the angle brackets look
...
single one in advance. Do not react while they write them.

Then run it once, and go down the list together comparing. Ask about the ones they got
wrong; skip the ones they got right.

The four questions at the bottom, in order of value:

1. **What decides which job `+` does?** The one that matters. They should get there.
2. **Find the other division operator.** `//`. They will find it by guessing or by asking
   what the opposite of `/` is. `100 // 3` gives `33`. Ask where the third went.
3. **`int("12.5")`** — predict, then run. It raises `ValueError`, met as b6.
   The lesson: `int` converts *text that spells a whole number*, and "12.5" does not.
   This is the exact bug that will bite them in Practice 5.
...
this. Say plainly that it is not a bug in their code, not a bug in Python, that every
...
the day they meet it in anger they will remember seeing it before.

Task 4 asks them to keep adding dashes until the ring closes, and to write down how many
blocks of four lines they pasted. That number goes in the Journal. It is the single best
argument for Area 1 and it is better made by their own hands than by you.
...
Entry 4. Good prompt for tonight if they are stuck for *what broke*: **which prediction
were you most confident about and most wrong about?**
...
1. Get to `p4e2` early. If the type lab is dragging, cut its last two questions and
   draw something.
2. Let them do the choice board before finishing the lab if they want to. The lab's
   questions survive being answered in the wrong order.
3. If it is really flat, stop after `p4e1` and split the session. Types get their real
   lesson next session anyway, under pressure, when `input` breaks their program.

**It is fine for this session to be the weak one.** Practices 3, 5 and 6 carry the area.
Do not fight to make tonight great at the cost of them arriving at Practice 5 sour.
